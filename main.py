...
def predict_pos_neg(review):
    ...
    if (predict_score > 0.5):
        print('[{}] => {:.2f}% 활률로 긍정리뷰 예상'.format(review, predict_score * 100))
    else:
        print('[{}] => {:.2f}% 활률로 부정리뷰 예상'.format(review, (1 - predict_score) * 100))

predict_pos_neg(review_list[750][1])
